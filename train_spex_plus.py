...
class LightningModule(pl.LightningModule):
    def __init__(self, config):
        super().__init__()
        self.model = SpEx_Plus(
            **config['model']
        )
        self.config = config
        self.neg_sisdr_loss = neg_sisdr_loss_wrapper
    ...
    def training_step(self, batch, batch_idx):
        ...
        enrollment_length = batch['enrollment_length']
        output, logits = self.model(mixture, enrollment, enrollment_length)

        neg_sisdr_loss = self.neg_sisdr_loss(output.unsqueeze(1), target)
        ce_loss = self.ce_loss(logits, label)
        loss = neg_sisdr_loss + self.config['loss']['gamma'] * ce_loss

        self.log('train_loss', loss, on_step=False, on_epoch=True, prog_bar=True, sync_dist=True, batch_size=mixture.size(0))
        self.log('train_neg_sisdr_loss', neg_sisdr_loss, on_step=False, on_epoch=True, prog_bar=True, sync_dist=True, batch_size=mixture.size(0))
        self.log('train_ce_loss', ce_loss, on_step=False, on_epoch=True, prog_bar=True, sync_dist=True, batch_size=mixture.size(0))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        enrollment = batch['enrollment']
        enrollment_length = batch['enrollment_length']
        output, _ = self.model(mixture, enrollment, enrollment_length)

        neg_sisdr_loss = self.neg_sisdr_loss(output.unsqueeze(1), target)
        loss = neg_sisdr_loss

        self.log('val_loss', loss, on_step=False, on_epoch=True, prog_bar=True, sync_dist=True, batch_size=mixture.size(0))
        self.log('val_neg_sisdr_loss', neg_sisdr_loss, on_step=False, on_epoch=True, prog_bar=True, sync_dist=True, batch_size=mixture.size(0))
