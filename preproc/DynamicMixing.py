"""
This module is initially proted from: https://github.com/khanld/Dynamic-Mixing, with modifications to adapt to TGIF's requirement.
"""
...
class DynamicMixing:
    def __init__(self,
        ...
        Args:
            ...
            max_bb_to_mix: The maximum number of babble noise to be mixed with the clean source. Default is 3
            fix_num_speakers: Set "True" to fix number of speaker per mixture.
        ...
        self.max_bb_noise_to_mix = max_bb_noise_to_mix
        self.fix_num_speakers = fix_num_speakers
        ...
        self.sub_sample_length = round(sub_sample_duration * sr)
        self.silence_length = int(silence_length * self.sr)
        self.saved_dir = saved_dir
        os.makedirs(os.path.join(self.saved_dir, 'clean'), exist_ok=True)
        os.makedirs(os.path.join(self.saved_dir, 'noisy'), exist_ok=True)
    ...
    def select_speaker_y(self, target_length, clean_path):
        temp_bb_noise_dataset_list = set(self.bb_noise_dataset_list)
        temp_bb_noise_dataset_list = list(temp_bb_noise_dataset_list - set([clean_path]))
        assert len(self.bb_noise_dataset_list) - len(temp_bb_noise_dataset_list) == 1
    ...
    def get_speaker_id(self, path):
        ...
        if filename.startswith("book"):
            speaker_id = path.split("_")[-2]
        elif filename.startswith("eng_vctk"):
            speaker_id = path.split("_")[-1].split(".")[0]
        return speaker_id
    ...
    def generate(self, file_id, clean_path, save_to_dir = False):
        
        clean_y = self.load_wav(clean_path)        
        clean_y = self.subsample(clean_y, sub_sample_length=self.sub_sample_length)

        sirs = []
        speakers_y = []
        bb_noise_files = []
        if len(self.bb_noise_dataset_list) > 0 and self.max_bb_noise_to_mix > 0:
            if not self.fix_num_speakers:
                # n_bb_noises = np.random.randint(0, self.max_bb_noise_to_mix + 1)
                n_bb_noises = random.choice(list(range(self.max_bb_noise_to_mix + 1)))
            else:
                n_bb_noises = self.max_bb_noise_to_mix
            while len(speakers_y) < n_bb_noises:
                speaker_y, bb_noise_file = self.select_speaker_y(len(clean_y), clean_path)
        ...
        noises_y = []
        bg_noise_files = []
        if len(self.bg_noise_dataset_list) > 0 and self.max_bg_noise_to_mix > 0:
            if self.allowed_overlapped_bg_noise:
        ...
        clean_speaker_id = self.get_speaker_id(clean_path)
        bb_noise_speaker_ids = list(map(self.get_speaker_id, bb_noise_files))
        speaker_ids = "-".join([clean_speaker_id, *bb_noise_speaker_ids])

        clean_filename = f"fileid_{file_id}_spkid_{speaker_ids}.wav"
        if save_to_dir:
            clean_saved_path = os.path.join(self.saved_dir, 'clean', clean_filename)
            noisy_saved_path = os.path.join(self.saved_dir, 'noisy', clean_filename)
            self.audiowrite(clean_saved_path, clean_y)
            self.audiowrite(noisy_saved_path, noisy_y)


        output = {
            "clean": clean_y,
            "noisy": noisy_y,
            "clean_file_path": clean_saved_path,
            "noisy_file_path": noisy_saved_path,
            "bg_noise_files": bg_noise_files,
            "bb_noise_files": bb_noise_files,
            "sirs": sirs,
            "snrs": snrs,
            "rir_file": rir_file,
            "num_speakers": len(speaker_ids.split("-")),
        }
        return output
