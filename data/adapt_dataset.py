...
def get_dataloaders(config, is_ddp=False, world_size=1, rank=0):
    ...
    train_dataset = TGIFDataset(
        ...
        enrollment_length=config['dataset']['enrollment_length'],
        enrollment_root_dir=config['dataset']['enrollment_root_dir'],
...
def get_enrollment_sample(speaker_to_files, current_file_path, subset='tr', enrollment_root_dir=None, mode='train'):
    """
    ...
    # elif subset == 'tt':
    #     print('############### im here')
    enrollment_file = os.path.join(enrollment_root_dir, f'enrol_eng_vctk_{speaker_id}.wav')
...
class TGIFDataset(Dataset):
    def __init__(
        ...
        self.enrollment_root_dir = enrollment_root_dir
    ...
    def __getitem__(self, idx):
        ...
        enrol_path = get_enrollment_sample(self.speaker_to_files, clean_path, subset=self.set_name, enrollment_root_dir=self.enrollment_root_dir)
        ...
        spex_plus_path = clean_path.replace('/clean/', '/spex_plus/')
        speakerbeam_path = clean_path.replace('/clean/', '/speakerbeam/')
        spex_plus_waveform, _ = torchaudio.load(spex_plus_path)
        speakerbeam_waveform, _ = torchaudio.load(speakerbeam_path)
        ...
        if self.segment_length is not None:
            ...
            spex_plus_waveform = spex_plus_waveform[:, offset: offset + self.segment_length]
            speakerbeam_waveform = speakerbeam_waveform[:, offset: offset + self.segment_length]

        if self.enrollment_length is None:
            enrol_waveform, _ = torchaudio.load(enrol_path)
        else:
            enrol_waveform, _ = torchaudio.load(enrol_path, frame_offset=0, num_frames=self.enrollment_length)
        ...
        sample = {
            ...
            'spex_plus': spex_plus_waveform,
            'speakerbeam': speakerbeam_waveform,
